...
for exp_name in ["standard", "rhetorics"]:
    ...
    for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        if exp_name == "rhetorics":
            instruction = " For this task use zeugma, join both senteces by the same word and if the zeugma preserves the same sense (doesn't sound like a bad pun), they are identical, otherwise they are differente."
        else:
            instruction = ""
        prompt = f"""
        Given two different sentences your task is to determine if a word has a identical
        or a different meaning between usages.{instruction} After thinking give, the answer in the
